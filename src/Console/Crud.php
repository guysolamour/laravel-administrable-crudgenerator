...
namespace Guysolamour\Administrable\Crudgenerator\Console;
...
use Guysolamour\Administrable\Crudgenerator\Console\Commands\Generate\BaseGenerate;
...
class Crud
{
    ...
    public function generateFieldIsAllowedFor(BaseGenerate $class_name): bool
    {
        ...
        /**
         * @var \Guysolamour\Crudgenerator\Console\Crud $this
         */
    ...
    private function setActions(array $data): void
    {
        ...
        if (!is_string($actions) && !is_array($actions)) {
            throw new CrudGeneratorException("The actions must be an array or string separated with [|] or [,]. Only one can be used.");
        }
    ...
    private  function hydrateFields(array $data): void
    {
        ...
        if (empty($fields)) {
            throw new CrudGeneratorException("You have to defined some fields for the [{$this->model}] model.");
        }
    ...
    private function exec(string $class_name)
    {
        ...
        /**
         * @var \Guysolamour\Crudgenerator\Console\Commands\Generate\BaseGenerate
         */
    ...
    {
        return $this->exec("\Commands\Generate\Generate" . Str::ucfirst($type));
    }

    public function append(string $type)
    {
        return $this->exec("\Commands\Append\Append" . Str::ucfirst($type));
    }

    public function rollback(string $type)
    {
        return $this->exec("\Commands\Rollback\Rollback" . Str::ucfirst($type));
    }
